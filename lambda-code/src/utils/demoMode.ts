 ...
 */
export const getDemoResponse = (claimType: DemoClaimType, requestId?: string): AnalysisResponse => {
  ...
  const responses: Record<DemoClaimType, AnalysisResponse> = {
    supported: {
      ...
      confidence_score: 85,
      recommendation:
        'This claim is well-supported by credible sources. Multiple peer-reviewed studies and authoritative sources confirm the information.',
      progress_stages: [
      ...
      sift_guidance:
        'Stop: Verified by credible sources. Investigate: Multiple peer-reviewed sources confirm. Find: Consensus among experts. Trace: Original research is well-documented.',
      timestamp,
    },
    ...
    disputed: {
      ...
      confidence_score: 75,
      recommendation:
        'This claim is disputed by credible sources and fact-checkers. Multiple authoritative sources contradict the information presented.',
      progress_stages: [
        { stage: 'Claim Extraction', status: 'completed', timestamp },
        { stage: 'Source Retrieval', status: 'completed', timestamp },
        { stage: 'Evidence Synthesis', status: 'completed', timestamp },
      ...
      sift_guidance:
        'Stop: Do not share. Investigate: Multiple fact-checkers dispute this. Find: Seek authoritative sources. Trace: Original claim lacks credible sourcing.',
      timestamp,
    },
    ...
    unverified: {
      ...
      confidence_score: 30,
      recommendation:
        'Unable to verify this claim with credible sources. Apply the SIFT framework before sharing.',
      progress_stages: [
        { stage: 'Claim Extraction', status: 'completed', timestamp },
        { stage: 'Source Retrieval', status: 'completed', timestamp },
        { stage: 'Evidence Synthesis', status: 'completed', timestamp },
      ...
      misinformation_type: null,
      sift_guidance:
        'Stop: Unverified claim. Investigate: Source credibility unknown. Find: Seek better coverage from credible sources. Trace: Cannot locate original authoritative source.',
      timestamp,
    },
    ...
    manipulated: {
      ...
      confidence_score: 90,
      recommendation:
        'Evidence suggests this content has been manipulated or fabricated. Do not share.',
      progress_stages: [
        { stage: 'Claim Extraction', status: 'completed', timestamp },
        { stage: 'Source Retrieval', status: 'completed', timestamp },
        { stage: 'Evidence Synthesis', status: 'completed', timestamp },
      ...
      sift_guidance:
        'Stop: Do not share manipulated content. Investigate: Forensic analysis confirms manipulation. Find: Seek original unaltered source. Trace: Content has been digitally altered.',
      timestamp,
    },
    ...
    biased: {
      ...
      confidence_score: 70,
      recommendation:
        'Content is factually accurate but uses selective framing or bias. Read better coverage for balanced perspective.',
      progress_stages: [
        { stage: 'Claim Extraction', status: 'completed', timestamp },
        { stage: 'Source Retrieval', status: 'completed', timestamp },
        { stage: 'Evidence Synthesis', status: 'completed', timestamp },
      ...
      misinformation_type: 'Misleading Content' as MisinformationType,
      sift_guidance:
        'Stop: Consider the framing. Investigate: Facts are accurate but presentation is biased. Find: Read multiple perspectives. Trace: Seek more balanced coverage.',
      timestamp,
    },
  };
 ...
 * @returns Appropriate demo response with grounding data
 */
export const getDemoResponseForContent = (content: string): AnalysisResponse => {
  const lowerContent = content.toLowerCase();

  // Get grounding bundle for content
  const groundingBundle = getDemoGroundingBundle(content);

  // Convert grounding sources to evidence sources
  const credible_sources: EvidenceSource[] = groundingBundle.sources.slice(0, 5).map((source) => ({
    url: source.url,
    title: source.title,
    snippet: source.snippet,
    why: `Credible source from ${source.domain}`,
    domain: source.domain,
  }));

  // Create SIFT details with evidence URLs
  const evidenceUrls = credible_sources.map((s) => s.url);
  const sift: SIFTDetails = {
    stop: {
      summary: 'Pause before sharing. Verify the source and check for credible evidence.',
      evidence_urls: evidenceUrls.slice(0, 2),
    },
    investigate: {
      summary:
        'Check the source credibility and look for corroborating evidence from trusted outlets.',
      evidence_urls: evidenceUrls.slice(0, 3),
    },
    find: {
      summary: 'Search for better coverage from authoritative news sources and fact-checkers.',
      evidence_urls: evidenceUrls.slice(0, 3),
    },
    trace: {
      summary: 'Trace the claim back to its original source to verify authenticity.',
      evidence_urls: evidenceUrls.slice(0, 2),
      earliest_source: evidenceUrls[0],
    },
  };
  ...
  const grounding: GroundingMetadata = {
    providerUsed: 'demo',
    sources_count: groundingBundle.sources.length,
    latencyMs: 0,
    attemptedProviders: ['demo'],
    sourcesCountRaw: groundingBundle.sources.length,
    sourcesCountReturned: credible_sources.length,
    cacheHit: false,
  };

  // Check for specific example claims first (exact matches for demo UI)
  let baseResponse: AnalysisResponse;
  
  if (lowerContent.includes('eiffel tower') && lowerContent.includes('paris')) {
    baseResponse = getDemoResponse('supported');
  } else if (lowerContent.includes('moon landing') && lowerContent.includes('faked')) {
    baseResponse = getDemoResponse('disputed');
  } else if (lowerContent.includes('new species') && lowerContent.includes('discovered')) {
    baseResponse = getDemoResponse('unverified');
  } else if (
    lowerContent.includes('fake') ||
    lowerContent.includes('manipulated') ||
    lowerContent.includes('photoshop')
  ) {
    baseResponse = getDemoResponse('manipulated');
  } else if (
    lowerContent.includes('disputed') ||
    lowerContent.includes('false') ||
    lowerContent.includes('debunk')
  ) {
    baseResponse = getDemoResponse('disputed');
  } else if (
    ...
    baseResponse = getDemoResponse('biased');
  } else if (
  ...
  // Add grounding data to response
  return {
    ...baseResponse,
    credible_sources,
    sift,
