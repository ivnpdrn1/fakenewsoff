...
export async function exampleAnalysisPipeline(
  ...
  try {
    ...
    const synthesis = await synthesizeEvidence(claimResult.claims, sources, ragChunks);
    ...
    const labelResult = await determineLabel(claimResult.claims, synthesis, mediaAnalysis);
    ...
    if (error instanceof ServiceError) {
      ...
      if (!error.retryable) {
        return {
          ...
          sift_guidance:
            'Apply SIFT framework: Stop, Investigate the source, Find better coverage, Trace claims.',
          sources: [],
          misinformation_type: null,
        };
      }
 ...
 */
export async function exampleLabelWithFallback(claims: any[], synthesis: any) {
